...
-- kor 저장 동시에 total 실행되기 때문에 이전 kor로 total 계산됨
...
desc stuscore3;
-- invisible  (total name 뒤로 옮기기 위함)
...
select * from stuscore3;
-- 별칭
select kor+eng as ke from stuscore3 s; -- as 생략가능
select * from stuscore3;
...
-- Q1. 전체평균보다 이하인 학생들에게만 전체평균값을 입력하시오.
update stuscore3
...
-- Q1. 근무일수에서 +100일 후 보너스 지급할 예정 출력
...
-------------------- 문제풀이 ----------------------
-- 등가비교연산자 / 논리연산자 and or not / (not) in / is (not) null / between A and B
---------------------------
select * from employees;

-- Q2. 월급 4000이상이면서 부서번호가 50이상인 사원을 검색하시오. and
select * from employees where salary>=4000 and department_id>=50 order by employee_id;
-- Q3. 사원번호 150번 이상이거나 입사일이 2007/01/01 이전인 사원을 검색하시오. or
select count(*) from employees;
select employee_id,hire_date from employees where employee_id>=150 or hire_date<='2007-01-01';

-- Q4. 월급 4000,6000,7000인 사원을 검색하시오. (not)in / in(not)null
...
-- Q5. 부서번호가 50번인 사원을 검색하시오. =
select department_id from employees where department_id=50;

-- Q6. 부서번호가 20,30번이 아닌 사원을 검색하시오. (not)in / !=
select department_id from employees where department_id not in(20,30) order by department_id;
select department_id from employees where not department_id != 20 or department_id != 30;

-- Q7. 월급이 4000이상 6000이하인 사원 검색하시오. >= / and / between A and B
select * from employees where salary>=4000 and salary<=6000;
select * from employees where salary between 4000 and 6000;

-- Q8. 월급 3800이상, job_id가 sh_clerk인 사원 검색하시오.
select * from employees where salary>=3800 and job_id='SH_CLERK'; -- 데이터는 대소문자 구분

-- Q9. CLERK 포함되어 있는 사원을 모두 검색하시오. like '%name%';
select job_id from employees where job_id like '%CLERK%';
-- Q10. 이름 중에 n이 들어가 있는 사원을 출력하시오. like '%n%'
select * from employees where emp_name like '%n%';
-- Q11. 이름 중에 2번째 o가 들어가 있는 사원이름을 검색하시오. (like '_o%')
select emp_name from employees where emp_name like '_o%';
-- Q12. 이름 중에 3번째 u가 들어가 있는 사원이름을 검색하시오.
select emp_name from employees where emp_name like '__u%';

----------------------------
select * from stuscore;

-- Q1. 이름 중에 순이 들어간 학생을 검색하시오. like '%n%';
select name from stuscore where name like '%순%';
-- Q2. 이름 2번째 a가 들어있는 학생을 검색하시오. like '_n%';
select name from stuscore where name like '_a%';

----------------------------
select * from employees;

-- Q1. job_id에 _ 있는 사원을 검색하시오. like "%#_%' escape'#';
select emp_name,job_id from employees where job_id like '%#_%' escape '#';

-- Q2. 입사일이 00/01/01 ~ 07/12/31 인 사원을 검색하시오.
select emp_name, hire_date from employees where hire_date>='00-01-01' and hire_date<='07-12-31';
select emp_name, hire_date from employees where hire_date between '00/01/01' and '07/12/31';

---------------------------
-- Q1. member테이블에서, 날짜 25년 1~3월에 가입한 회원을 출력하시오.
select * from member;

select * from member where mdate >='25/01/01' and mdate <='25/03/31' order by mdate asc;
select * from member where mdate between '25/01/01' and '25/03/31' order by mdate asc;

-- Q2. 이름 중에 3번째 g가 들어간 사람을 검색하시오.
select * from stuscore;
select * from stuscore where lower(name) like '__g%';  -- 다 소문자로 바꾼 후 검색(대소문자 상관없이 검색됨)

-- Q3. salary, 년봉, 1438원, 한화 천단위 표시
--    -> 대소문자 구분없이 3번째 g 들어가 있는 사원만 출력하시오.
select * from employees;
...
-- Q4. emp_name 이름순으로 정렬하시오.  order by
select * from employees order by emp_name asc;   -- asc  순차정렬(오름차순)
select * from employees order by emp_name desc;  -- desc 역순정렬(내림차순)

-- Q5. email E 또는 S 포함되어 있는 사원을 출력하시오. (대문자)
select * from employees where email like '%E%' or email like '%S%' order by email;
select email from employees where email like '%E%' and email like '%S%' order by email desc;

-- Q6. kor 순차, eng/math 역순정렬
--   2개 order by 정렬, 1차 우선 정렬 후 같은 값일 경우,
--   2차 정렬방식을 적용 2차 값이 같을 때 3차 방식 적용
select * from stuscore;
select * from stuscore order by kor asc, eng desc, math desc; -- 순서: kor-eng-math
